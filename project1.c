...
int isGameOver(char board[6][7]){
  ...
  int columnChecker();
  int diagonalChecker();
  int noMoves = availableMoves(board);
...
//available moves, checks to see if there are any possible moves
int availableMoves(char board[6][7]){
  int moves=0;
  for(int i = 0; i<7;i++){
    if(board[0][i] == '.'){
      moves++;
...
int aiMove(char board[6][7], char aiChar, char playerChar){
	...
	for(int i=0; i<7; i++){
		...
		int breakPoint = 0;   // Break point -- DELETE THIS
		...
		int breakPoint2 = 0;   // Break point -- DELETE THIS
	}
